...
#End Region
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        ElseIf ComboBox1.Text = "Customer Cost, Sales and Profit" Then
            cmd.CommandText = "with CTE (Cname, Cost, Sales, Profit, ProPerc) As (SELECT        View_SalesInv_Main.CUST_CODE+' | '+View_SalesInv_Main.CUST_NAME As CName, SUM(View_SalesInv_Profit.Cost) AS Cost, SUM(View_SalesInv_Profit.Sales) AS Sales, SUM(View_SalesInv_Profit.Profit) AS Profit, Convert(decimal(18,2),Case When SUM(View_SalesInv_Profit.Profit)=0 Then 0 Else SUM(View_SalesInv_Profit.Profit)*100/SUM(View_SalesInv_Profit.Cost) End) As ProPerc FROM View_SalesInv_Profit INNER JOIN View_SalesInv_Main ON View_SalesInv_Profit.INV_NO =  View_SalesInv_Main.INV_NO Where View_SalesInv_Main.INV_DATE >= '" + dtpFDate.Value.Date.ToString + "' and View_SalesInv_Main.INV_DATE <= '" + dtpToDate.Value.Date.AddDays(1).ToString + "' GROUP BY View_SalesInv_Main.CUST_NAME,View_SalesInv_Main.CUST_CODE) Select * from CTE Union Select 'Sum Total', Sum(Cost), Sum(Sales), Sum(Profit), Convert(decimal(18,2),Case When Sum(Profit)=0 Then 0 Else Sum(Profit)*100/Sum(Cost) End) from CTE"
